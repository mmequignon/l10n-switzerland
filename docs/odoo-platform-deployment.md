...
* `docker-compose.yml`
 ...
 ```
 odoo:
  image: camptocamp/enfinfidu_odoo:10.0.0
   ...
   environment:
     ...
     - STATSD_CUSTOMER=enfinfidu_odoo
...
* `rancher.public.env`
 ...
 export AWS_BUCKETNAME=enfinfidu-odoo-integration
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=enfinfidu-odoo-integration
...
* `docker-compose.yml`
 ...
 letsencrypt:
   ...
   image: janeczku/rancher-letsencrypt:v0.3.0
   volumes:
     - enfinfidu-prod-certs:/etc/letsencrypt/production/certs
 ```
...
* `rancher.public.env`
 ...
 export AWS_BUCKETNAME=enfinfidu-odoo-prod
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=enfinfidu-odoo-prod
...
```
./rancher enfinfidu-odoo-integration up -d letsencrypt
...
* Use the local `./rancher` script, example to start the containers:
  ...
  ```
  ./rancher enfinfidu-odoo-integration up -d
  ```
...
* Use the local `./rancher` script, example to upgrade the containers:
  ...
  ```
  ./rancher enfinfidu-odoo-integration up --pull --recreate --confirm-upgrade -d
  ```
