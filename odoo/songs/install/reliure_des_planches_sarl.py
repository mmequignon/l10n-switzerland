...
def setup_company(ctx):
    create_or_update(
        ctx, 'res.company', '__setup__.reliure_des_planches_sarl',
        {
            'name': u'Reliure des Planches Sarl',
    ...
    company = ctx.env.ref('__setup__.reliure_des_planches_sarl')
    ...
    if not partner.get_metadata()[0]['xmlid']:
        add_xmlid(ctx, partner, '__setup__.reliure_des_planches_sarl_partner')
...
def import_users(ctx):
    ...
    content = resource_stream(
        req,
        'data/install/reliure_des_planches_sarl/res.users.csv')
    load_csv_stream(ctx, 'res.users', content, delimiter=',')

    user = ctx.env.ref('__setup__.res_user_reliure_des_planches_sarl_manager')
    user.password_crypt = (
...
def import_account_journals(ctx):
    ...
    fp = 'data/install/reliure_des_planches_sarl/account.journal.csv'
...
def import_payment_modes(ctx):
    ...
    fp = 'data/install/reliure_des_planches_sarl/account.payment.mode.csv'
    content = resource_stream(req, fp)
...
def import_partner_banks(ctx):
    """ Import payment modes """
    fp = 'data/install/reliure_des_planches_sarl/res.partner.bank.csv'
    content = resource_stream(req, fp)
...
def add_customer_company_to_main_company_users(ctx):
    ...
    for user in ctx.env['res.users'].search([
        ...
        user.write(
            {'company_ids': [(4, ctx.env.ref(
                '__setup__.reliure_des_planches_sarl'
            ).id)]})


@anthem.log
def configure_reliure_des_planches_sarl_chart_of_account(ctx):
    """Configure Missing COA for Reliure_Des_Planches_Sarl"""
...
def main(ctx):
    ...
    configure_reliure_des_planches_sarl_chart_of_account(ctx)
