...
class IrActionsReport(models.Model):
    ...
    def merge_invoice_pdf_in_memory(self, docs):
        streams = []
    ...
    def render_qweb_pdf(self, res_ids=None, data=None):
        ...
        invoice_pdf_io = BytesIO(invoice_pdf)

        isr_report = self._get_report_from_name("l10n_ch.isr_report_main")
        isr_pdf, _ = isr_report.render_qweb_pdf(res_ids, data)
        isr_pdf_io = BytesIO(isr_pdf)

        qr_report = self._get_report_from_name("l10n_ch.qr_report_main")
        qr_pdf, _ = qr_report.render_qweb_pdf(res_ids, data)
        qr_pdf_io = BytesIO(qr_pdf)

        pdf = False
        if self.report_name == reports[0]:
            pdf = self.merge_invoice_pdf_in_memory([invoice_pdf_io, isr_pdf_io])
        elif self.report_name == reports[1]:
            pdf = self.merge_invoice_pdf_in_memory([invoice_pdf_io, qr_pdf_io])
        else:
            pdf = self.merge_invoice_pdf_in_memory([invoice_pdf_io, isr_pdf_io, qr_pdf_io])
