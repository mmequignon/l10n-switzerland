...
class PaymentISR(common.TransactionCase):
    ...
    def test_payment_non_isr_grouping_single_supplier(self):
        ...
        """
        invoices = (
            self.create_supplier_invoice(self.supplier_isrb1, 'INV1')
            | self.create_supplier_invoice(self.supplier_isrb1, 'INV2')
        )
        # create an invoice where ref is set instead of invoice_payment_ref
        PaymentRegister = self.env['account.register.payments']
        ctx = {'active_model': 'account.invoice', 'active_ids': invoices.ids}
        ...
        expected_vals = [
            ...
            ('INV1 INV2', self.supplier_isrb1.id, 2, 84.0),
        ]
        self.assertEqual(
            [(v['communication'], v['partner_id'], len(v['invoice_ids'][0][2]),
                v['amount']) for v in sorted(vals, key=lambda i: (i['communication'], i['partner_id']))],
    ...
    def test_payment_non_isr_single_supplier(self):
        ...
        No grouping on free ref should apply
        ...
        expected_vals = [
            # ref, partner, invoice count, amount
            # 2 invoices ungrouped
            ('INV1', self.supplier_isrb1.id, 1, 42.0),
            ('INV2', self.supplier_isrb1.id, 1, 42.0),
        ]
        self.assertEqual(
            [(v['communication'], v['partner_id'], len(v['invoice_ids'][0][2]),
                v['amount']) for v in sorted(vals, key=lambda i: (i['communication'], i['partner_id']))],
