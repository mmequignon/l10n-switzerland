...
if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
  ...
  if [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    docker push "camptocamp/enfinfidu_odoo:${TRAVIS_TAG}"
  elif [ ! -z "$TRAVIS_BRANCH" ]; then
    echo "Deploying image to docker hub for branch ${TRAVIS_BRANCH}"
    docker tag enfinfidu_odoo camptocamp/enfinfidu_odoo:${TRAVIS_BRANCH}
    docker push "camptocamp/enfinfidu_odoo:${TRAVIS_BRANCH}"
  else
